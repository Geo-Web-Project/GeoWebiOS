...
class FramedImageSystem : System {
    ...
    func update(context: SceneUpdateContext) {
        context.scene.performQuery(Self.query).forEach { entity in
            ...
            guard let imageAssetUrl = imageCom.imageAssetUrl else { return }
            ...
            let physicalHeightInMillimeters = Float(imageCom.physicalWidthInMillimeters) * Float(texture.height) / Float(texture.width)
            let plane = MeshResource.generatePlane(width: Float(imageCom.physicalWidthInMillimeters) / 1000, height: physicalHeightInMillimeters / 1000)
